<?php
    //archivo para insertar los datos del formulario de usuarios-administracion.php en la BBDD
    ...
    if (!empty($nombre) && !empty($apellidos) && !empty($email) && !empty($telefono) && !empty($fnac) && !empty($usuario) && !empty($pass)) {
            ...
            if ($insertar_uno == true) {
                ...
                foreach($roles as $rolesStr) {
                    $rolesStr = mysqli_real_escape_string($mysqli, $rolesStr);
                }
            ...
            if ($insertar_dos == true) {
                //redirige a usuarios-administracion.php y muestra mensaje de confirmación
